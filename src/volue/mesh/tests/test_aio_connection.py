...
from volue.mesh import Timeseries, from_proto_guid, to_proto_curve_type
...
async def test_get_timeseries_entry_async():
    ...
    async with connection.create_session() as session:
        try:
            ...
            test_cases = [test_case_1, test_case_2, test_case_3]
            for test_case in test_cases:
                entry = await session.get_timeseries_resource_info(**test_case)
...
@pytest.mark.asyncio
@pytest.mark.database
async def test_update_timeseries_attribute_async():
