...
def main(address, port, tls_root_cert):
    ...
    with connection.create_session() as session:
        ...
        session.update_rating_curve_versions(
            target=rating_curve_attribute_path,
            start_time=versions[0].valid_from_time,  # use first version `valid_from_time` as start interval
        ...
        session.update_rating_curve_versions(
            target=rating_curve_attribute_path,
            start_time=versions[-1].valid_from_time,  # to replace old version use its `valid_from_time` as start interval
            end_time=datetime.max,  # this is the last version
